...
db.restaurants.find({name : /^Wil/}).pretty()
db.restaurants.find({name : /.*ces$/}).pretty()
db.restaurants.find({name : /.*Reg.*/}).pretty()
...
db.restaurants.find({ $or: [{ "cuisine": "Seafood", "cuisine": { $nin: ["American", "Chinese"] } }, { "name": /^Wil/ }]}, { "restaurant_id": 1, "name": 1, "borough": 1, "cuisine": 1})
db.restaurants.find({ "grades": { $elemMatch: { "grade": "A", "score": 11, "date": ISODate("2014-08-11T00:00:00Z")}}}, { restaurant_id: 1, name: 1, grades: 1, _id: 0}).pretty()
...
db.restaurants.find({ "address.coord.1": { $gt: 42, $lte: 52 } }, { restaurant_id: 1, name: 1, address: 1, coord: 1 , _id: 0}).pretty()
...
db.restaurants.find({ "name": /mon/i }, { "name": 1, "borough": 1, "address.coord": 1, "cuisine": 1 }, {_id : 0}).pretty()
